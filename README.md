...
│                                                            │
│  ┌────────────────┐                                       │
│  │ data/generator  │  seeded PRNG → 32 fictional accounts  │
│  │  (in-memory)    │  each with billing + usage + tickets  │
│  └───────┬────────┘                                       │
│          │                                                 │
│          ▼                                                 │
│  ┌────────────────┐        ┌───────────────────────────┐  │
│  │  scoring/       │───────▶  tools/                    │  │
│  │  riskScore.ts   │        │  list_accounts             │  │
│  │  (weights +     │        │  get_account_details       │  │
│  │   drivers)      │        │  list_at_risk_accounts     │  │
│  └───────┬────────┘        │  get_account_risk_summary  │  │
│          │                  └───────────────────────────┘  │
│          │                                                 │
│          ▼                                                 │
│  ┌────────────────────────────┐                           │
│  │ resources/methodology.ts    │  methodology://risk-scoring│
│  │ (weights, thresholds, docs) │                           │
│  └────────────────────────────┘                           │
└────────────────────────────────────────────────────────────┘
```
